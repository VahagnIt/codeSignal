...
// and round it to 0 or to 10. If it's less than 5 we round it to 0 if it's larger than or equal to 5 we
// round it to 10 (rounding to 10 means increasing the next significant digit by 1). The process stops
...
// 1445 -> 1450 -> 1500 -> 2000.
function rounders(n) {
  let count = n.toString().length - 1;
  for (let i = 0; i < count; i++) {
    n = Math.round(n / 10);
  }
  return +(n + "0".repeat(count));
}
