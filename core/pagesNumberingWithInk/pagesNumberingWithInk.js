...
//  if it has the full number printed on it (e.g. if we are working with page 102 but have ink only for
//      two digits then this page will not be considered numbered).

// It's guaranteed that you can number the current page, and that you can't number the last one in the
//  book.
// Example
...
function pagesNumberingWithInk(current, numberOfDigits) {
  ...
  while (numberOfDigits - length >= 0) {
    ...
    length = current.toString().length;
  }
  return current - 1;
}
